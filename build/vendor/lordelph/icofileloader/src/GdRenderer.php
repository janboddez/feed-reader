    ...
    private function render32bit(IconImage $img, $gd)
    {
        ...
        for ($i = $img->height - 1; $i >= 0; --$i) {
            for ($j = 0; $j < $img->width; ++$j) {
                ...
                if ($alpha7 < 127) {
                    $col = $alpha7 << 24 | \ord($binary[$offset + 2]) << 16 | \ord($binary[$offset + 1]) << 8 | \ord($binary[$offset]);
    ...
    private function buildPalette(IconImage $img, $gd)
    {
        ...
        if ($img->bitCount != 24) {
            ...
            for ($i = 0; $i < $img->colorCount; ++$i) {
                $palette[$i] = $this->allocateColor($gd, $img->palette[$i]['red'], $img->palette[$i]['green'], $img->palette[$i]['blue'], \round($img->palette[$i]['reserved'] / 255 * 127));
            }
        }
        return $palette;
    }
