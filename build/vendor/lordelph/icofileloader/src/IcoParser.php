...
namespace FeedReader\Elphin\IcoFileLoader;
...
class IcoParser implements ParserInterface
{
    ...
    private function parsePNGAsIco($data)
    {
        ...
        $icoDirEntry = ['width' => $w, 'height' => $h, 'bitCount' => $bits];
    ...
    {
        for ($i = 0; $i < $count; ++$i) {
            $icoDirEntry = \unpack('Cwidth/Cheight/CcolorCount/Creserved/Splanes/SbitCount/LsizeInBytes/LfileOffset', $data);
    ...
    {
        $bitmapInfoHeader = \unpack('LSize/LWidth/LHeight/SPlanes/SBitCount/LCompression/LImageSize/' . 'LXpixelsPerM/LYpixelsPerM/LColorsUsed/LColorsImportant', \substr($data, $entry->fileOffset, 40));
